...
async function searchPokemonById(id){
    ...
    // arregle con los nombres de los pokemones
    ...
    pokemonSearched.forEach(pokemon =>{
        draggableElements.innerHTML += 
        `<div class="pokemon">
             <img id="${pokemon.name}" draggable="true" class="image" src="${pokemon.sprites.other['official-artwork'].front_default}" alt="imagen pokemon">
        </div>`
    })
    ...
    names.forEach(name => {
        ...
        name.addEventListener('drop',  event=>{
            ...
            if(event.target.innerText == draggableElementData){
                ...
                console.log('NO')
                loseMsg.innerText = 'Ups, te equivocaste!!'
            }
        })
    })
}
